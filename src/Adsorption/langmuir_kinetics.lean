...
This section defines the Langmuir adsorption model from kinetics for a single adsorbate: 
`θ = K*Pₐ/(1+K*Pₐ)` 
where:
...
## Assumption
The model assumes the rate of adsorption `r_ad = k_ad * Pₐ * S` and the rate of desorption `r_d = k_d * A` 
