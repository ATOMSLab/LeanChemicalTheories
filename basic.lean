...
def gaylussacs_law {𝕜 : Type u} [comm_group 𝕜] (M : ideal_gas 𝕜) :=  ∃ (k : 𝕜), ∀ n : ℕ, (pressure n)/(temperature n)= k
...
theorem gaylussacs_law_relation 
: gaylussacs_law M →  ∀ n m, (@pressure 𝕜 _ _ n) / temperature n = pressure m / temperature m:=
begin
  intros h n m,
  simp [gaylussacs_law] at h,
  cases h with k h,
  field_simp [h n, h m],
end

theorem gaylussacs_law_relation'
: (∀ n m, (@pressure 𝕜 _ _ n) / temperature n = pressure m / temperature m) → gaylussacs_law M :=
begin
  intros h,
  simp [gaylussacs_law],
  use (pressure 1 / temperature 1),
  intro,
  exact h n 1,
end
...
theorem gaylussacs_from_ideal_gas {𝕜 : Type u}[comm_group 𝕜] {M : ideal_gas 𝕜} (iso1 : isochoric M.to_thermo_system)
(iso2 : closed_system M.to_thermo_system) 
: gaylussacs_law M:=
begin
  simp [gaylussacs_law, isochoric, closed_system] at *,
  let h := @ideal_gas_law_relation 𝕜 _ M,
  apply gaylussacs_law_relation',
  intros,
  specialize h n m,
  rw [mul_comm (pressure n) _,mul_comm (pressure m) _]  at h,
  iterate 2 {rw mul_div_mul_comm at h,},
